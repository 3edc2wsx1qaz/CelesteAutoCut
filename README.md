...
剪辑算法会始终保留 `room_enter -> load_level` 的进房 intro 片段；即使后续没有找到可配对的 `load_level -> transition / strawberry_collect / level_complete` 成功段、而是直接 `exit`，这段 intro 也不会被丢掉。起始边界优先选择 `session_start`；没有 `session_start` 时选择第一个 `room_enter`；如果缺少 `room_enter`，会用第一个 load / `player_position_sample` 合成一个入口片段继续匹配。单独的末尾 `room_enter -> load_level` 片段会直接使用对应 `player_position_sample` 的时间戳作为终点。
