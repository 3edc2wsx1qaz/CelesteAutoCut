...
- OBS helper 默认轮询间隔为 `1000ms`；房间状态文件最多约每秒刷新一次；`room_enter -> load_level` 的进房 load 后人物坐标观察为每 20 帧一次、最多约 8 秒，并只把一个最佳 `player_position_sample` 写入当前 `room_event_<timestamp>.jsonl`；非进房 `load_level` 只在 load 后固定第 25 帧写入一次人物坐标样本。事件 jsonl 只保留剪辑判断需要的字段，删除冗余来源/原因/草莓属性/采样元信息，降低单行长度和磁盘写入量。
...
- 终点前 checkpoint 特例：同一房间内 `load_level(A) -> death -> load_level(B) -> transition` 也可算合法成功段，但成功段从 B 加载后固定第 25 帧的 `player_position_sample` 开始，避免保留死亡转场动画；A 与 B 的 `respawnPointX/Y` 必须存在且坐标不同，B->transition 中不能再出现额外 `death` / `load_end`；这个区间原因标记为 `checkpoint_death_successful_attempt`；
...
- 对单独的 `room_enter -> load_level` 进房片段，helper 会优先使用 load 后写入的最佳 `player_position_sample`，并把片段结束点放到该采样时间，原因标记为 `room_entry_load_player_position_end`；只有缺少 spawn point 或采样时才回退到固定 `PostRollMs` delay，原因标记为 `room_entry_load_delay`；
