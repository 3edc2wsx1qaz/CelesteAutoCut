...
| `Clip Intensity` / `剪辑强度` | 默认 `Low`。选择 `Low` 保留更宽松的 load->exit/录制末尾区间；选择 `High` 使用精确 player-position 切点和望远镜/对话后的进房采样。 |
...
- `Low`：默认模式，使用更宽松的保留规则。`load_level` 边界直接使用原始 `load_level` 时间戳；`room_enter -> load_level -> death -> load_level` 保持选择最初的 `room_enter -> load_level` 进房区间；最后一个未通关的进房段会保留 `load_level -> exit`，没有 `exit` 时保留 `load_level -> 录制末尾`。
